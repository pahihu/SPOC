...
/*}}}*/
/*{{{  Insert HEADER*/
typedef struct SF_P_loopback_loop tSF_P_loopback_loop;
void P_loopback_loop (tSF_P_loopback_loop *FP);
...
static struct Prototypes Table[] =
{
  {"void P_loopback_loop (tSF_P_loopback_loop *FP);","P_loopback_loop",sizeof(tSF_P_loopback_loop)},
...
int main(int argc, char**argv)
{
  ...
  else
  {
    int arg;
    int i;
    fprintf(f,"#ifndef __loop_INCLUDES__\n");
    fprintf(f,"#define __loop_INCLUDES__\n");
  ...
  while(true)
  {
    switch(FP->_Header.IP)
    {
      CASE(0):  
        ...
        GOTO(1);
      CASE(2):  
        INPUT1(FP->toLoop_59,&FP->_Header.Temp.VBYTE,3);
      CASE(3):  
        ...
        break;
      CASE(5):  
        INPUT4(FP->toLoop_59,&FP->EqnType_61,7);
      CASE(7):  
        INPUT4(FP->toLoop_59,&FP->Samples_62,8);
      CASE(8):  
        INPUT8(FP->toLoop_59,&FP->deltax_65,9);
      CASE(9):  
        INPUT8(FP->toLoop_59,&FP->Power_64,10);
      CASE(10):  
        INPUT4(FP->toLoop_59,&FP->Node_63,11);
      CASE(11):  
        GOTO(4);
      CASE(6):  
        FP->_Header.Temp.VBYTE = 4;
        OUTPUT1(FP->fromLoop_60,&FP->_Header.Temp.VBYTE,12);
      CASE(12):  
        FP->More_66 = false; 
        GOTO(4);
      CASE(4):  
